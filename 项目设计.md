...
| **方法** | **路径**                          | **说明** |
| ------ | ------------------------------- | ------ |
| GET    | `/api/v1/cart/current`          | 当前购物车  |
| POST   | `/api/v1/cart/scan`             | 扫码加购   |
| POST   | `/api/v1/cart/checkout`         | 结算     |
| GET    | `/api/v1/orders`                | 订单列表   |
| POST   | `/api/v1/orders/{id}/refund`    | 退款     |
| GET    | `/api/v1/products`              | 商品列表   |
| GET    | `/api/v1/members/phone/{phone}` | 会员查询   |
...
| **阶段**       | **周次** | **交付物**                                   |
| ------------ | ------ | ----------------------------------------- |
| **P0 骨架**    | W1     | 项目脚手架 + FastAPI + SQLite + 离线登录 + License |
| **P1 领域**    | W2     | Order/Cart/Product/Member 聚合 + 单写队列       |
| **P2 收银**    | W3     | 扫码 → 购物车 → 结账 → 落库（Null 硬件）               |
| **P3 支付**    | W4     | 现金/会员余额 + 支付回调                            |
| **P4 打印**    | W5     | 小票打印 + 开钱箱 + 客显                           |
| **P5 会员**    | W6     | 会员充值 + 积分 + 等级                            |
| **P6 硬件**    | W7     | 扫码枪/电子秤驱动 + 设备管理                          |
| **P7 后台**    | W8     | 商家后台 + 报表                                 |
| **P8 管理端**   | W9     | 管理后台 + 商户/授权管理                            |
| **P9 Agent** | W10    | 推荐/异常/库存 Agent                            |
| **P10 部署**   | W11    | 一键脚本 + 服务注册 + 离线包                         |
| **P11 测试**   | W12    | 单元/集成/E2E + 压测                            |

**架构承诺**：每一级只增加装配，不修改领域层。L0 到 L1 升级只需 `pip install -e ".[hardware,pay]"`。
...
| 1     | 无外部服务       | 仅本进程监听                  |
| 2     | 无管理员权限      | 普通用户可运行                 |
| 3     | 无 Python 环境 | Windows 便携版可运行          |
| 4     | 无网络         | 完整功能可用                  |
| 5     | 数据可迁移       | 复制 data 目录即可            |
| 6     | 卸载干净        | `cashier uninstall` 无残留 |
...
| **维度** | **v1.0** | **v2.0** | **v3.2**             |
| ------ | -------- | -------- | -------------------- |
| 框架     | Flask    | FastAPI  | FastAPI              |
| 架构     | 三层       | DDD 四层   | 确定性内核 + Agent 旁路     |
| 离线     | 部分       | 部分       | **完全离线**             |
| 认证     | JWT 在线   | JWT 在线   | **本地 bcrypt + HMAC** |
| 授权     | 静态字段     | 在线/离线混合  | **RSA 本地验签**         |
| 并发     | 无保障      | 幂等 + 乐观锁 | **+ 单写队列**           |
| Agent  | 无        | 无        | **规则引擎 + 统计基线**      |
| 内存     | \~120MB  | \< 80MB  | **\< 50MB（空闲）**      |
| 启动     | \~5s     | \< 3s    | **\< 2s**            |
| 依赖     | 15+      | 10       | **8**                |
| 部署     | 手动       | 手动       | **一条命令**             |
