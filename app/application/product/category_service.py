...
class CategoryService:
    ...
    def get_category(self, category_id: str) -> dict | None:
        with session_factory() as s:
            c = (
                s.query(CategoryModel)
                .filter_by(id=category_id, merchant_id=self.merchant_id)
                .first()
            )
    ...
    def update_category(self, cat_id: str, data: dict) -> dict | None:
        with session_factory() as s:
            c = (
                s.query(CategoryModel)
                .filter_by(id=cat_id, merchant_id=self.merchant_id)
                .first()
            )
            if not c:
                return None
    ...
    def seed_demo_categories(self) -> int:
        ...
        with session_factory() as s:
            existing = (
                s.query(CategoryModel).filter_by(merchant_id=self.merchant_id).count()
            )
            if existing > 0:
                return 0
