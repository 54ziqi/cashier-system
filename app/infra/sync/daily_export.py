...
def build_daily_digest(day: str | None = None,
                       merchant_id: str = "local",
                       store_name: str = "",
                       parent_merchant_id: str = "",
                       max_stores: int = 1) -> dict:
    """
        ...
        row = (
            s.query(
                ...
                __import__("sqlalchemy").func.coalesce(
                    __import__("sqlalchemy").func.sum(Order.final_amount), 0
                ).label("rev"),
            )
    ...
    return {
        ...
        "top5_categories": [{"name": r.product_name, "revenue": r.rev} for r in cat_rows],
        "generated_at": datetime.now(timezone.utc).isoformat(),
    }


def export_signed_digest(priv_pem: bytes,
