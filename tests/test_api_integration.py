...
class TestCheckoutAPI:
    ...
    def _login(self, client) -> str:
        """登录获取 admin token"""
        resp = client.post("/api/v1/auth/login", json={"username": "admin", "password": _TEST_ADMIN_PWD})
        assert resp.status_code == 200
        return resp.json()["token"]

    def _get_product(self, client, token: str, pid: str = "demo_p004") -> dict:
        """获取商品库存"""
        resp = client.get(f"/api/v1/merchant/products", headers={"Authorization": f"Bearer {token}"})
    ...
    def test_order_list(self, client):
        """订单列表"""
        token = self._login(client)
        resp = client.get("/api/v1/merchant/orders", headers={"Authorization": f"Bearer {token}"})
        assert resp.status_code == 200
        data = resp.json()
    ...
    def test_products_list(self, client):
        """商品列表"""
        token = self._login(client)
        resp = client.get("/api/v1/merchant/products", headers={"Authorization": f"Bearer {token}"})
        assert resp.status_code == 200
        data = resp.json()
        assert data["total"] > 0
